...
以上内容为master分支的主要内容，基本完成了ROS系统和STM32单片机通信的模板，并可以初步发布一些传感器消息。后续的重点是将此模板应用到实际的机器人底盘上。
...
- **2019/4/12 **
  1.  新建ros_balancecar branch，开始将master的通信模板应用到ROS平衡车的构建中。
...
- **2019/4/26 **
  ...
  2. **==思考==**：平衡车的直立控制如何做到在将小车提起时电机停转？

     思路1：在平衡的小角度范围关闭速度控制仅保留直立控制

     思路2：增加离地检测的传感器（如红外距离传感器）
...
- **2019/9/5 **
  ...
  1. 通过ROS的客户端服务器模型更改参数：
     ...
     - 使用方法：
       ...
       d. 在单片机和上位机建立rosserial连接之后，启用新的终端调用 rosparam call /pid 可以看到启动rosserial的终端中打印出“pidCallback回调函数测试”的日志。
